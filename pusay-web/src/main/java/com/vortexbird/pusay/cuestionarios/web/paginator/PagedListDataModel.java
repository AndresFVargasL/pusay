 ...
 * entire dataset, the managed bean should declare an inner class which extends
 * this class and implements the fetchData method. This method is called as
 * needed when the table requires data that isn't available in the current data
 * page held by this object. This does require the managed bean (and in general
 * the business method that the managed bean uses) to provide the data wrapped
 * in a DataPage object that provides info on the full size of the dataset.
 */
...
public abstract class PagedListDataModel<T> extends DataModel {
	 ...
	 * object currently cached doesn't include that index or the data is marked
	 * as dirty, then fetchPage is called to retrieve the appropriate page.
	 */
	@Override
